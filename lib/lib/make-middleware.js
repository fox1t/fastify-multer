...
function makeMiddleware(setup) {
    return function multerMiddleware(req, res, next) {
        if (!type_is_1.default(req, ['multipart'])) {
            return next();
        }
        const options = setup();
        ...
        let busboy;
        try {
            busboy = new busboy_1.default({ headers: req.headers, limits: limits, preservePath: preservePath });
        }
        catch (err) {
            return next(err);
        }
        const appender = new file_appender_1.default(fileStrategy, req);
        let isDone = false;
        ...
        function done(err) {
            ...
            on_finished_1.default(req, function () {
                next(err);
            });
        }
        ...
        busboy.on('field', function (fieldname, value, fieldnameTruncated, valueTruncated) {
            ...
            append_field_1.default(req.body, fieldname, value);
        });
        busboy.on('file', function (fieldname, fileStream, filename, encoding, mimetype) {
            ...
            fileFilter(req, file, function (err, includeFile) {
                ...
                storage._handleFile(req, file, function (error, info) {
                    if (aborting) {
                        appender.removePlaceholder(placeholder);
